...
TARGET_FIELDS = {
    "name", "fathers name", "date of birth", "pan number",
    "photo", "signature", "income tax department",
    "govt of india", "bharat sarkar", "ayakar vibhag",
    "pan label", "national", "tampered"
}
...
for jf in sorted(ANN_DIR.glob("*.json")):
    ...
    for item in items:
        ...
        if label not in TARGET_FIELDS: continue
...
print(f"{'Field':<30} {'x1':>6} {'y1':>6} {'w':>6} {'h':>6} {'std_x':>7} {'std_y':>7} {'n':>5}")
print("─" * 75)
template = {}
for label in sorted(field_positions.keys()):
    ...
    print(f"{label:<30} {avg[0]:>6.3f} {avg[1]:>6.3f} {avg[2]:>6.3f} {avg[3]:>6.3f} "
