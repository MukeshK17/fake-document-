...
def _fires(stage_cfg: dict[str, Any]) -> bool:
    ...
    return stage_cfg.get("enabled", True) and random.random() < float(stage_cfg.get("p", 0.5))
...
class DocumentPreprocessor:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self._aug_dispatch: list[tuple[str, Callable[[Image.Image, dict], Image.Image]]] = [
        ...
        logger.info(
            ...
            len(self._clean_stages), self._aug_enabled, self._target_size,
        )
    ...
    def _aug_random_rotation(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        return image.rotate(random.uniform(-max_a, max_a), expand=False, fillcolor=(255, 255, 255))
    ...
    def _aug_random_perspective(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        dst = src + np.array(
            [[random.uniform(-w * scale, w * scale),
              random.uniform(-h * scale, h * scale)] for _ in range(4)],
            dtype=np.float32,
        )
        M = cv2.getPerspectiveTransform(src, dst)
        return Image.fromarray(
            cv2.warpPerspective(arr, M, (w, h),
    ...
    def _aug_random_brightness(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        return ImageEnhance.Brightness(image).enhance(random.uniform(float(lo), float(hi)))
    ...
    def _aug_random_contrast(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        lo, hi = cfg.get("factor_range", [0.7, 1.3])
        return ImageEnhance.Contrast(image).enhance(random.uniform(float(lo), float(hi)))

    @staticmethod
    def _aug_random_gaussian_blur(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        """Random Gaussian blur — simulates low-DPI / out-of-focus capture."""

        lo, hi = cfg.get("radius_range", [0.5, 2.0])
        return image.filter(ImageFilter.GaussianBlur(radius=random.uniform(float(lo), float(hi))))

    @staticmethod
    def _aug_random_jpeg_compression(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
    ...
    def _aug_random_noise(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        return Image.fromarray(
            np.clip(arr + np.random.normal(0.0, random.uniform(float(lo), float(hi)), arr.shape),
    ...
    def _aug_random_ink_bleed(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        arr = np.array(image)
        return Image.fromarray(cv2.bitwise_not(cv2.dilate(cv2.bitwise_not(arr), kernel)))
    ...
    def _aug_cutout(image: Image.Image, cfg: dict[str, Any]) -> Image.Image:
        ...
        for _ in range(int(cfg.get("num_holes", 1))):
            hole_h = random.randint(1, max(1, int(img_h * float(cfg.get("max_h_ratio", 0.2)))))
            hole_w = random.randint(1, max(1, int(img_w * float(cfg.get("max_w_ratio", 0.2)))))
            y1, x1 = random.randint(0, img_h - hole_h), random.randint(0, img_w - hole_w)
    ...
    def _apply_orientation_correction(bgr: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            angle, conf = int(osd.get("rotate", 0)), float(osd.get("orientation_conf", 0.0))
    ...
    def _apply_denoise(bgr: np.ndarray) -> np.ndarray:
        ...
        return cv2.fastNlMeansDenoisingColored(bgr, None, h=10, hColor=10,
    ...
    def _apply_clahe(bgr: np.ndarray) -> np.ndarray:
        ...
        lab_norm = cv2.merge([cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(l_ch),
                              a_ch, b_ch])
        return cv2.cvtColor(lab_norm, cv2.COLOR_LAB2BGR)

    @staticmethod
    def _resize_with_padding(bgr: np.ndarray, target_size: tuple[int, int]) -> np.ndarray:
        ...
        canvas[(th - nh) // 2:(th - nh) // 2 + nh,
               (tw - nw) // 2:(tw - nw) // 2 + nw] = cv2.resize(bgr, (nw, nh), interpolation=cv2.INTER_AREA)
        return canvas
