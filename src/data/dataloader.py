...
def _build_weighted_sampler(dataset: DocumentDataset) -> WeightedRandomSampler:
    ...
    logger.debug("WeightedSampler | %s",
...
def build_dataloaders(
    ...
    seq_len = int(config.get("layoutlmv3", {}).get("processor", {}).get("max_seq_length", 512))
    ...
    val_loader  = _eval_loader(val_manifest,  config, "val",  eval_batch_size, num_workers, pin_memory, collate)
    test_loader = _eval_loader(test_manifest, config, "test", eval_batch_size, num_workers, pin_memory, collate)

    logger.info("DataLoaders | train=%d (%d batches) | val=%d | test=%d",
