...
class DocumentDataset(Dataset):
    ...
    def __init__(
        self, manifest_path: str | Path, config: dict[str, Any], split: str = "train"
    ) -> None:
        ...
        logger.info(
            "DocumentDataset [%s] | %d samples | labels=%s",
            split,
            len(self._samples),
            sorted(self.label2id),
        )
    ...
    def __getitem__(self, idx: int) -> DocumentSample:
        ...
        if label_str not in self.label2id:
            raise ValueError(
                f"Unknown label '{label_str}' at row {idx}. "
    ...
    def _load_manifest(self) -> list[dict[str, str]]:
        ...
        raise ValueError(
            f"Unsupported manifest format '{suffix}'. Expected .csv or .jsonl"
        )
        ...
        with self._manifest_path.open(encoding="utf-8") as fh:
            for i, line in enumerate(fh, 1):
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid JSON on line {i} of {self._manifest_path}"
                    ) from exc
                rows.append(
                    {
    ...
    def _load_ocr_cache(self) -> dict[str, dict[str, Any]]:
        cache_path = self._manifest_path.with_name(
            self._manifest_path.stem + "_ocr_cache.jsonl"
        )
