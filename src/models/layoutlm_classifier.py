...
_REQUIRED_LM_KEYS = {"pretrained_model_name_or_path", "id2label", "processor", "inference"}
...
class LayoutLMv3Classifier:
    ...
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.id2label: dict[int, str] = {int(k): v for k, v in self._cfg["id2label"].items()}
        self.label2id: dict[str, int] = {v: k for k, v in self.id2label.items()}

        logger.info("LayoutLMv3Classifier | device=%s | labels=%s",
                    self.device, list(self.id2label.values()))
    ...
    def inference(
        ...
        use_amp = self._rt_cfg.get("mixed_precision", False) and self.device.type == "cuda"
        with torch.no_grad():
        ...
        return {
            ...
            "scores":     {self.id2label[i]: round(float(probs[i].item()), 4)
    ...
    def _validate_ocr_inputs(words: list[str], boxes: list[BoundingBox]) -> None:
        if len(words) != len(boxes):
            raise ValueError(f"words({len(words)}) and boxes({len(boxes)}) length mismatch.")
