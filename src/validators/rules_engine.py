...
class RulesEngine:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self._pan_regex = re.compile(
            v.get("pan_card", {}).get("pan_regex", r"^[A-Z]{5}[0-9]{4}[A-Z]{1}$")
        )
        self._name_min_len = int(v.get("pan_card", {}).get("name_min_length", 3))
        self._aadhaar_digits = int(v.get("aadhar_card", {}).get("digit_count", 12))
        self._bal_tolerance = float(
            v.get("bank_statement", {}).get("balance_tolerance_inr", 1.0)
        )
    ...
    def _validate_pan(
        self, text: str, words: list[str], _boxes: list
    ) -> dict[str, bool]:
        pan_match = bool(re.search(self._pan_regex, text))
        has_name = any(
            len(w) >= self._name_min_len and w.isalpha() and w.isupper() for w in words
        )
    ...
    def _validate_aadhaar(
        self, text: str, _words: list, _boxes: list
    ) -> dict[str, bool]:
        ...
        has_uid_label = bool(
            re.search(r"unique\s+identification|aadhaar|आधार", text, re.IGNORECASE)
        )
        has_dob = bool(
            re.search(r"DOB|Year of Birth|\d{2}/\d{2}/\d{4}", text, re.IGNORECASE)
        )
        return {
    ...
    def _validate_bank(self, text: str, _words: list, _boxes: list) -> dict[str, bool]:
        has_account = bool(
            re.search(
                r"account\s*(no|number|#)?\.?\s*:?\s*\d{9,18}", text, re.IGNORECASE
            )
        )
    ...
    def _validate_salary(
        ...
        has_employer = bool(
            re.search(r"employer|company|organisation|pvt|ltd|llp", text, re.IGNORECASE)
        )
        has_employee = bool(re.search(r"employee|name\s*:", text, re.IGNORECASE))
        has_net_pay = bool(
            re.search(r"net\s*pay|net\s*salary|take\s*home", text, re.IGNORECASE)
        )
        has_month = bool(
            re.search(
                r"january|february|march|april|may|june|july|august|september|october|november|december|salary\s*slip",
                text,
                re.IGNORECASE,
            )
        )
    ...
    def _validate_itr(self, text: str, _words: list, _boxes: list) -> dict[str, bool]:
        ...
        has_ay = bool(
            re.search(r"assessment\s+year|A\.?Y\.?\s*20\d{2}", text, re.IGNORECASE)
        )
        has_income = bool(
            re.search(r"total\s+income|gross\s+total", text, re.IGNORECASE)
        )
        has_signature = bool(
            re.search(
                r"e-?verification|digital\s+signature|acknowledgement",
                text,
                re.IGNORECASE,
            )
        )
